...
RUN set -eux \
    ...
    && mv /opt/utils/docker-entrypoint.sh /opt/portkey-docker-entrypoint.sh \
    && chmod +x /opt/portkey-docker-entrypoint.sh \
...
CMD [ "/opt/portkey-docker-entrypoint.sh" ]
