...
[[ "$started_at" =~ ^[0-9]{4}-[0-9]{2}-[0-9]{2}T ]] || { echo "missing verification start time" >&2; exit 2; }

for report in "$single_report" "$ha_report" "$redis_report" "$migration_report" "$concurrency_report"; do
...
jq -e --arg commit "$commit" --arg run_id "$run_id" --arg started_at "$started_at" '
  ...
  (.tested_at | type == "string" and . >= $started_at) and
